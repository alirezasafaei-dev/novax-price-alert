...
## وضعیت فعلی و جهت‌گیری تولید (Production)

- تمام ویژگی‌های پایه و hardening تثبیت شده و در تولید فعال هستند.
- تمرکز فعلی: بهبود مستمر رابط کاربری (TWA و بات)، کارایی، قابلیت‌های غنی‌تر برای کاربران واقعی ایرانی، و گسترش هوشمند دارایی‌ها و ابزارها بدون محدودیت‌های MVP.
- metrics و مانیتورینگ عملیاتی: endpoint سبک `/metrics` و داشبورد JSON در `/metrics/summary` فعال و در حال استفاده.
- تاریخچه‌ی قیمت: endpoint خواندنی `/api/v1/prices/history` و چارت تعاملی کامل در TWA (با انتخاب بازه زمانی و مقایسه) فعال.
- دارایی‌ها و بازارها: پشتیبانی از ارزهای فیات، طلا/سکه، و کریپتوهای اصلی (BTC, ETH, BNB و ...) با mappingهای کامل. گسترش تدریجی بر اساس تقاضای کاربر و کیفیت داده.
- UX کامل: TWA به عنوان اپلیکیشن غنی با بخش‌های My Assets (دارایی‌های من)، Suggestions (پیشنهادهای هوشمند)، تاریخچه چارتی، و ویزارد ساخت هشدار پیشرفته. بات متنی هم با کیبوردهای غنی و لینک مستقیم به TWA یکپارچه شده.
- هیچ محدودیتی به عنوان "MVP" یا "nice-to-have" باقی نمانده؛ همه چیز به عنوان محصول تولید واقعی در نظر گرفته می‌شود و بهبودهای بزرگ در اولویت هستند.
